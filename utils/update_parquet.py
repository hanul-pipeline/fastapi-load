...
def update_parquet(date:str, time:str):
    ...
    query = """SELECT `날짜`, DATE_FORMAT(`시간`, %s) as `시간`, `위치ID`, `분류ID`, `센서ID`, `측정값` 
                FROM measurements 
                WHERE `날짜` = %s AND `시간` BETWEEN SUBTIME(%s, '02:00:00') AND %s"""
    params = ('%H:%i:%s', date, time, time)
    ...
    df.to_parquet(DIR, engine='pyarrow', index=False, partition_cols=['날짜', '센서ID'])
